...
def main():
    # タイトルと説明
    st.title('水文水質DB 数値取得Webアプリ')
    st.text('水文水質データベースの雨量・流量・水位取得を行うためのWebアプリケーションを作成してみました。\n雨量情報を取得する機会があればぜひお使いください。')

    st.markdown("""
    ### 本アプリについて
    水文水質DBの値を年単位で取得できるWebアプリです。

    ### 取得手順
    1. 取得したいデータに種類を選択します。雨量、流量、水位から選んでください。
    2. 単年で取得するか、複数年を一括で取得するか選択してください。複数年の場合はcsvファイルのダウンロードになります。
    3. 取得したい"観測所記号"を調べます。[例）厳木ダム](http://www1.river.go.jp/cgi-bin/SiteInfo.exe?ID=109021289922040)
    4. 取得したい年数を入力します。予め取得できる年数を確認しておくことをお勧めします。
    """)

    # プルダウンでデータの種類を選択
    data_type = st.selectbox(":red[▼データの種類を選択]", ["雨量", "流量", "水位"])
